...
const teamsModel = {
  ...
  getAllTeams: async () => {
    const query = `
      ...
      GROUP BY t.team_id, t.team_name, t.project_id, p.name, 
               t.team_head_id, u.name, ed.designation, d.name, t.created_at, t.updated_at
  ...
  createTeam: async (team_data) => {
    const { team_name, team_head_id, project_id } = team_data;
    
    const query = `
      INSERT INTO teams (team_name, team_head_id, project_id)
      VALUES ($1, $2, $3)
      RETURNING *
    `;
    
    const values = [team_name, team_head_id || null, project_id || null];
  ...
  updateTeam: async (team_id, team_data) => {
    ...
    const values = [team_name, team_head_id || null, project_id || null, team_id];
    const result = await pool.query(query, values);
    return result.rows[0];
  },
